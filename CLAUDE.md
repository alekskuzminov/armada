...
```bash
cd /var/www/armada && git pull && npm ci && rm -f .next/lock && npm run build && cp -r public .next/standalone/public && cp -r .next/static .next/standalone/.next/static && pm2 restart armada
```
